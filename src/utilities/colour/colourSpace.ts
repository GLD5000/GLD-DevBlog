export const colourSpace = {
  ...
  convertHextoRgbString(hex: string) {
    const splitHex = colourSpace.splitHexString(hex);
    const rgbArray = splitHex.map((digits) =>
      colourSpace.hexDigitsToInteger(digits)
    );
  ...
  convertSrgbToHslArray(srgbArray: Array<number>) {
    ...
    sat = Math.max(
      0,
      Math.min(1, delta === 0 ? 0 : delta / (1 - Math.abs(2 * lum - 1)))
    );
    sat = +(sat * 100);
    lum = +(lum * 100);
  ...
  convertHslArrayToHex(hslArray: Array<number>) {
    return colourSpace.convertSrgbToHex(
      colourSpace.convertHslArrayToSrgb(hslArray)
    );
  },
  convertSrgbToHex(srgbArray: Array<number>) {
    return srgbArray.reduce(
      (acc, curr) => `${acc}${colourSpace.decimalToHex(curr)}`,
      "#"
    );
  },
  convertRgbToHex(srgbArray: Array<number>) {
    return srgbArray.reduce(
      (acc, curr) => `${acc}${colourSpace.integerToHex(curr)}`,
      "#"
    );
  },
